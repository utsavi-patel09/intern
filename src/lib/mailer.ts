...
/**
 * Generates the HTML template for the New Account Welcome email.
 */
export function getWelcomeTemplate(name: string, email: string, password: string) {
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; max-width: 600px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 24px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
      <div style="text-align: center; margin-bottom: 32px;">
        <div style="display: inline-block; width: 64px; height: 64px; background: linear-gradient(135deg, #1E3A5F 0%, #0EA5E9 100%); border-radius: 16px; padding: 12px;">
      ...
      <h2 style="color: #0f172a; text-align: center; font-size: 24px; font-weight: 800; margin-bottom: 8px; letter-spacing: -0.025em;">Welcome to InternHub!</h2>
      <p style="color: #64748b; text-align: center; font-size: 16px; margin-bottom: 32px;">Hi ${name}, your professional account has been successfully created. Use the following credentials to access your portal:</p>
      
      <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 24px; border-radius: 16px; margin-bottom: 32px;">
        <div style="margin-bottom: 16px;">
          <label style="display: block; font-size: 12px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 4px;">Email Address</label>
          <span style="font-family: 'Courier New', Courier, monospace; font-size: 18px; font-weight: 700; color: #1E3A5F;">${email}</span>
        </div>
        <div>
          <label style="display: block; font-size: 12px; font-weight: 800; color: #94a3b8; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 4px;">Temporary Password</label>
          <span style="font-family: 'Courier New', Courier, monospace; font-size: 18px; font-weight: 700; color: #0EA5E9;">${password}</span>
        </div>
      </div>
      
      <div style="text-align: center; margin-bottom: 32px;">
        <a href="${process.env.NEXTAUTH_URL}/login" style="display: inline-block; padding: 16px 32px; background: linear-gradient(135deg, #1E3A5F 0%, #0EA5E9 100%); color: white; text-decoration: none; font-weight: 800; border-radius: 12px; box-shadow: 0 4px 14px 0 rgba(30, 58, 95, 0.30);">Access Your Portal</a>
      </div>
      
      <div style="background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 16px; margin-bottom: 32px; border-radius: 4px;">
        <p style="color: #92400e; font-size: 14px; margin: 0;"><strong>Security Reminder:</strong> Please change your password immediately after logging in for the first time.</p>
      </div>
