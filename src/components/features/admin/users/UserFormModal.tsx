...
export function UserFormModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-0">
      ...
      <div className="relative bg-white/90 backdrop-blur-xl rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh]">
        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white/50 sticky top-0 z-10">
          <h3 className="text-xl font-bold font-heading text-slate-900 flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-indigo-100/50 flex items-center justify-center text-indigo-500">
              <svg className="w-5 h-5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
            </div>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-5">
            <div className="flex flex-col gap-1.5">
              ...
              {formik.touched.name && formik.errors.name && (
                <p className="text-rose-500 text-xs font-bold mt-1">{formik.errors.name}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                onChange={formik.handleChange}
                onBlur={formik.handleBlur}
                className={`form-input bg-white/80 focus:bg-white ${formik.touched.email && formik.errors.email ? 'border-rose-300 ring-rose-100' : ''}`}
              />
              {formik.touched.email && formik.errors.email && (
                <p className="text-rose-500 text-xs font-bold mt-1">{formik.errors.email}</p>
              )}
            </div>
            ...
            {formik.values.role === "intern" && (
              <div className="flex flex-col gap-1.5 animate-in slide-in-from-top-2 fade-in duration-300">
                ...
                <input
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  className={`form-input bg-white/80 focus:bg-white ${formik.touched.college && formik.errors.college ? 'border-rose-300 ring-rose-100' : ''}`}
                />
                {formik.touched.college && formik.errors.college && (
                  <p className="text-rose-500 text-xs font-bold mt-1">{formik.errors.college}</p>
                )}
              </div>
            )}
            ...
            <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-slate-100">
              ...
              <button
                ...
                className="btn-primary px-6 shadow-lg shadow-indigo-500/25 min-w-[140px] flex justify-center disabled:opacity-70"
              >
                {createLoading ? (
