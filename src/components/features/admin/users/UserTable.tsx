      ...
      <table className="w-full text-left">
        ...
        <tbody className="divide-y divide-slate-100">
          {users.length === 0 ? (
            ...
            users.map((user) => (
              <tr key={user.id} className="table-row group bg-white/40 hover:bg-white/80 transition-colors">
                <td className="table-td py-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-800 text-base group-hover:text-indigo-700 transition-colors">{user.name}</span>
                ...
                <td className="table-td py-4">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wider
                    ${user.role === 'admin' ? 'bg-rose-50 text-rose-700 border border-rose-100 group-hover:bg-rose-100' :
                      user.role === 'manager' ? 'bg-indigo-50 text-indigo-700 border border-indigo-100 group-hover:bg-indigo-100' :
