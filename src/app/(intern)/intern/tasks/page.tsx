    ...
    <div className="w-full max-w-7xl mx-auto px-4 lg:px-8 py-8 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
        ...
        <div className="flex items-center gap-3">
          <div className="card-glass px-4 py-2 flex items-center gap-2 bg-white/40">
                ...
                <tr>
                  <td colSpan={4} className="text-center py-16 text-slate-500 bg-white/30 backdrop-blur-sm">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="font-semibold text-slate-600">Youre all caught up!</p>
                  ...
                  <tr key={task.id} className="table-row group bg-white/40 hover:bg-white/80 transition-colors">
                    <td className="table-td py-5">
                      <div className="flex flex-col gap-1.5 maxWidth-xs xl:max-w-md">
                        <span className="font-bold text-slate-800 text-base group-hover:text-indigo-700 transition-colors">{task.title}</span>
                    ...
                    <td className="table-td py-5">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider shadow-sm border
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${task.status === 'completed' ? 'bg-emerald-500' : task.status === 'in_progress' ? 'bg-blue-500 animate-pulse' : 'bg-amber-500'}`}></span>
                    ...
                    <td className="table-td py-5 text-right w-48">
                      <div className="relative inline-block text-left w-full">
                        <select
                          className="form-select w-full bg-white/80 text-sm font-semibold border-slate-200 shadow-sm focus:border-indigo-400 focus:ring-indigo-400/20 py-2 cursor-pointer"
                          value={task.status}
